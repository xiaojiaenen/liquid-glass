...
export function GlassDatePicker({ value, onChange, width = 280 }: GlassDatePickerProps) {
  ...
  return (
    <LiquidGlass
      ...
      style={{ width, padding: 20, flexDirection: 'column' }}
    >
      {/* 头部 */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <button
          onClick={prevMonth}
          style={{ border: 'none', background: 'none', color: '#fff', fontSize: 16, cursor: 'pointer', padding: '4px 8px', opacity: 0.7 }}
        >
          ‹
        </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2, marginBottom: 8 }}>
        {WEEKDAYS.map((d) => (
          <div key={d} style={{ textAlign: 'center', fontSize: 11, opacity: 0.5, fontFamily: fontStack, padding: '4px 0' }}>
            {d}
          </div>
        ))}
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                background: active ? 'rgba(10,132,255,0.45)' : 'transparent',
                border: isToday && !active ? '1px solid rgba(255,255,255,0.3)' : '1px solid transparent',
